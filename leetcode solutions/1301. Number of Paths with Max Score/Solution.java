...
/**
 * Intuition
 * we will solve this problem using recursion and memoization
 * we will call recursion on bottom right cell
 * our base condition if cell conatins E, it will return path sum as 0 and paths
 * as 1
 * our base condition if cell contains X, it will return path sum as 0 and paths
 * as 0
 * then we will call recursion on top cell, left cell and left diagonal cell
 * we will fetch each of their scores
 * then we will compare all these scores and store in bestScore and bestPaths
 * At the end we will return bestScore and bestPaths as a res
 * Approach
 * 
 * Complexity
