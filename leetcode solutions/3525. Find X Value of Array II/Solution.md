...
- First we will traverse all queries
- Then for each node we will update value at the given index
- Then we will query node from start to n-1
- Then we will fetch frequency of given remainder or x and append in res, later return that res
- merging the nodes logic will be
- each node will have two values, first product of all its values, second frequency of remainders from 0 to k-1
- we will copy left child freq in root
- new remainder will be obtained when we multiply left product * x
- then we will update new remainder with freq of x, bcs whatever will be the frequency of x that many new remainder will be formed 
