...
- we will first store index of each intervals in an array
- then we will sort the the array based on the start time
- then we will store next index for every position in an array, it will tell the next start time of the next interval to take
- then we will call solve function, it will either skip the curr interval or take the curr interval
- if skip score is greater than take score add skip to the res
- if skip score is less than take score add take to the res
- if the score of both take and skip are equal, we will take lexicographically smaller indicies of skip or take
- Finally return res
