...
/**
 * Intuition
 * for k more than block size ie sqrt(n), we will solve through simulation
 * for k less than block size, we will store all queries in a map with key as k
 * we will traverse map, then we will traverse all queries for the curr k
 * we will compute all queries and store product values in diff array, then we
 * will update nums using this diff array
 * At last we will take xor of all nums and store in res
 * Finally return res
 * Approach
 * 
 * Complexity
